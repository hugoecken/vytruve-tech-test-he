...
- [ ] T033 [P] [US2] Add only the bounded shared `CollectionLoadError` and `CollectionRecoveryAlert` primitives in `apps/web/src/shared/ui/collection-recovery/`, then implement the patients TanStack Table definition with the page in the query key, response-derived pagination, `keepPreviousData` while pending, loading/empty/blocking-read-error/degraded-refresh states, retry actions, and pointer/keyboard row-open behavior without an action column in `apps/web/src/modules/patients/ui/patient-table/` (FR-012, FR-019, FR-045–FR-047, FR-050, FR-052–FR-053, FR-057; SC-001, SC-010, SC-017)
...
- [ ] T036 [US2] Add professional English and French patient-directory, creation, pagination, empty, blocking read failure, degraded refresh, retry, and ownership-safe copy in `apps/web/src/shared/i18n/locales/en/patients.json` and `apps/web/src/shared/i18n/locales/fr/patients.json` (FR-047–FR-048, FR-050, FR-052, FR-054, FR-057; SC-008, SC-010, SC-012, SC-017)
...
- [ ] T044 [P] [US3] Implement the scans TanStack Table definition with the page in the query key, response-derived pagination, `keepPreviousData` while pending, loading/empty/blocking-read-error/degraded-refresh states, retry actions, pointer/keyboard read-only row consultation without a consultation icon or scan identifier, independent download behavior, and print-eligibility action state in `apps/web/src/modules/scans/ui/scan-table/` (FR-022, FR-028, FR-030–FR-031, FR-045–FR-047, FR-050, FR-052–FR-053, FR-057; SC-004, SC-010, SC-017)
...
- [ ] T046 [US3] Add professional English and French scan, validation, storage, download, blocking read failure, degraded refresh, and retry translations in `apps/web/src/shared/i18n/locales/en/scans.json` and `apps/web/src/shared/i18n/locales/fr/scans.json` (FR-023, FR-047–FR-050, FR-052, FR-057; SC-008, SC-010, SC-017)
...
- [ ] T054 [P] [US4] Implement the print-request TanStack Table definition, canonical status badges, estimated-progress display, pointer/keyboard read-only row consultation without an action column or consultation icon and with text-only Estimated progress, independent page-keyed persisted-list and provider-status TanStack Query state, response-derived pagination, `keepPreviousData` while pending, active-row status/progress skeletons during status refresh with terminal values retained, and loading/empty/blocking-read-error/degraded-refresh recovery in `apps/web/src/modules/printing/ui/print-table/` (FR-035–FR-046, FR-050, FR-052–FR-053, FR-057; SC-006–SC-007, SC-010, SC-017)
...
- [ ] T056 [US4] Keep manual print-request refresh always available, run one automatic status refresh after each persisted page first loads, keep the refresh control visually idle during that automatic read and pending only after explicit activation, omit polling, use direct TanStack Query error handling, and preserve TanStack-retained rows when a status refresh fails in `apps/web/src/modules/printing/ui/print-table/` (FR-035–FR-050, FR-057; SC-006–SC-008, SC-010, SC-017)
- [ ] T057 [US4] Add professional English and French printing lifecycle, progress, capacity, confirmation-pending, failure, blocking read failure, degraded refresh, and retry translations in `apps/web/src/shared/i18n/locales/en/printing.json` and `apps/web/src/shared/i18n/locales/fr/printing.json` (FR-035–FR-050, FR-052, FR-057; SC-006–SC-008, SC-010, SC-017)
...
- [ ] T059 Audit all mutation and query call sites under `apps/web/src/modules/` for the accepted retry policy, direct TanStack Query state usage, explicit invalidation or refresh ownership, no focus/remount refetches or development-only request cancellations, blocking read recovery, background-refresh preservation, and absence of raw generated/server/provider messages (FR-047–FR-051, FR-057; SC-008, SC-010–SC-011, SC-017)
